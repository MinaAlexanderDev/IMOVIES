...
class FakeApi : MoviesApi {
    ...
    private var isMovieDetails: Boolean = false
    override suspend fun getPopularMovies(
        page: Int, pageSize: Int, apiKey: String, language: String
    ): MoviesResponse {
        return MoviesResponse(page = 1, totalPages = 10, totalResults = 100, results = movies)
    }
    ...
    override suspend fun getMovieDetails(
        ...
        return if (!isMovieDetails) {
            Response.success(
                movieDetails,
                okhttp3.Response.Builder().code(200).message("Response.success()")
                    .protocol(Protocol.HTTP_1_1)
                    .request(Request.Builder().url("http://test-url/").build())
                    .receivedResponseAtMillis(1619053449513).sentRequestAtMillis(1619053443814)
                    .build()
            )
        } else {
            Response.error(
    ...
    fun setIsMovieDetails(isMovieDetails: Boolean) {
        this.isMovieDetails = isMovieDetails
